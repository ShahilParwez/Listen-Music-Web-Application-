  ...
  useEffect(() => {
    (async () => {
      const response = await fetch(
        `https://api.napster.com/v2.2/albums/${album_id}?apikey=${NAPSTER_API_KEY}`
      );
      ...
      const song_response = await fetch(
        `https://api.napster.com/v2.2/albums/${album_id}/tracks?apikey=${NAPSTER_API_KEY}`
      );
        ...
        <Stack direction="column" spacing={4}>
          ...
          <Stack direction="row">
            {albumDetails?.tags?.map((single_tag, idx) => {
              return <Chip label={single_tag} key={idx} variant="outlined" />;
            })}
          </Stack>
        </Stack>
      </Stack>
