...
const  ResponsiveDrawer = (props) => {
  ...
  const container =
    ...
    const search = (e) => {
      ...
      const search_query = formData.get("search_key");
      navigate(`/search/${search_query}`);
    };

    const toggleTheme = (e) => {
      props.updateTheme(!e.target.checked);
    };
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      >
          ...
          <Box>
            <form onSubmit={search}>
              <Search sx={{ m: 1 }}>
                ...
                <StyledInputBase
                  placeholder="Search…"
                  inputProps={{ "aria-label": "search", name: "search_key" }}
                  sx={{ width: "30vw" }}
                />
              </Search>
            </form>
        ...
        <FormControlLabel
          control={<MaterialUISwitch sx={{ mr: 2 }} onChange={toggleTheme} />}
          label=""
        />
      </AppBar>
      <Box
